#!/usr/bin/env python3
"""
Simple Gun Detection with YOLO
Real-time gun detection using webcam - standalone version without backend alerts
"""
...
class SimpleGunDetector:
    def __init__(self, 
                 model_path: str = "best.pt",
                 confidence_threshold: float = 0.5):
        """
        Initialize the Simple Gun Detector
        ...
        logger.info(f"🔫 Simple Gun Detector initialized")
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            self.model = torch.hub.load('ultralytics/yolov5', 'custom', 
                                      path=model_path_abs, trust_repo=True)
        ...
        for *box, conf, cls in results.xyxy[0].cpu().numpy():
            if conf >= self.confidence_threshold:
                ...
                if class_name in ['gun', 'knife']:  # Only process weapons
                    ...
                    # Log detection
                    logger.warning(f"🚨 DETECTION: {class_name.upper()} detected! Confidence: {conf:.2%}")
        
        return detections
    
    def run(self, camera_index: int = 0, save_detections: bool = False):
        """Main detection loop"""
        logger.info(f"🚀 Starting Simple Gun Detection...")
        ...
        try:
            while True:
                ...
                # Add status overlay
                status_text = f"Gun Detection Active | Frame: {frame_count} | Detections: {self.detection_count}"
                ...
                # Display video feed
                cv2.imshow('Simple Gun Detection - YOLO', frame)
...
def main():
    parser = argparse.ArgumentParser(description='Simple Gun Detection with YOLO')
